...
`server/` 内部以**域**为第一层目录。**每个域目录承载该域自己负责的业务能力、决策逻辑与数据访问实现**。这并不意味着一个完整业务动作只能发生在一个域内部；当一个动作天然涉及多个域时，应保持各域内部实现各自内聚，同时通过明确的命令、事件、读模型或过程编排完成协作，而不是把多个域的职责强行揉进同一个目录。

在此前提下，业务逻辑和数据访问实现都保持在各自域目录下，没有独立的 `infra/` 或 `infrastructure/` 目录：
...
**以域为第一层级的价值**：理解或修改某个域自身负责的完整逻辑，只需要读 `server/order/` 这一个文件夹。该域内部的聚合、命令、事件、数据访问全部内聚；若一个业务动作跨多个域，则分别在对应域中维护各自职责，并通过显式协作连接起来，对人和 AI 都友好。
...
当从 `reference/engine/automated-execution.md` 进入时，走以下流程。

### 核心理念

技术栈决策权在用户手中。AI 的角色是分析、归纳和推荐，不是决定。

**决策顺序：**
1. 用户有明确要求 → 遵循
2. 无明确要求，有现有代码 → 分析后向用户呈现，确认后写入
3. 无明确要求，无现有代码 → 提供 2-3 个方案，用户选择后写入
4. 用户明确声明"不关心，你来定" → AI 自行选择后写入（标注原因）

### 第一步：判断用户是否有明确要求

```
检查 facts.md 或对话历史中是否包含明确的技术栈要求？
    │
    ├── 有（如 "用 Go + PostgreSQL"、"Node.js + Prisma"）
    │     → 直接遵循，不调研 / 不推荐 / 不提问替代方案
    │     → 要求不完整时（只指定了语言，未指定框架+数据库）：
    │         对缺失项提供 2-3 个补充建议，由用户决定
    │
    └── 无明确要求
          → 进入第二步
```
...
1. 一次性扫描全部配置文件（package.json / pyproject.toml / go.mod / Cargo.toml / docker-compose.yml / CI 配置），提取技术栈全貌
2. 抽样 5-10 个源文件识别命名风格、目录组织、错误处理和测试约定
3. 向用户呈现分析结果，标注每条信息来源：
   - `[配置文件]`：从配置文件中直接读取
   - `[代码推断]`：从代码模式中归纳
   - `[社区约定]`：由语言/框架主流约定直接确定
   - `[待确认]`：代码中有歧义或找不到足够证据
4. 用户确认后写入 tech-stack.md。用户有修改意见时，修改后重新呈现直至确认
5. 用户确认后才进入 D3-auto

### 无现有代码路径

1. 根据 facts.md 业务场景，调研 2-3 个可行方案
2. 向用户呈现方案，每个标注适合理由和取舍
3. 用户选择后写入完整 tech-stack.md
4. 用户选择后才进入 D3-auto

### 用户声明"不管"时

当用户明确说出"不关心技术选型，你来定"或等价表述时，AI 可以自行选择方案并写入 tech-stack.md，但必须在文件中记录"由 AI 自主选择"的原因，以便后续复查。

### D2-auto 完成后的行为

tech-stack.md 写入完成后，编排者继续执行 D3-auto（subagent 编排实现），不等待用户再次发起。
...
用户确认后，写入 `docs/vcddd/tech-stack.md`（见 `reference/guides/templates/tech/tech-stack.md`）。
